...
- **代码收尾四件套**（2026-09-11）：① Journal 孤儿记录清理（两路径都消失的记录启动时清除）+ 撤销容错（外部丢失的文件跳过而非整批失败）；② 模板/项目名/排序方式持久化（重启恢复）；③ 拍摄时间排序（F-05 排序基石：EXIF 优先、无时间排后、自然文件名序，网格头部可切换）；④ 键盘快捷键（⌘↩ 执行、⇧⌘Z 撤销上一批）

- **Duplicate Timestamp 的 Sequence 消解**（2026-09-11，Phase 4 收尾）：模板无 {index} 且连拍同秒时，同名资产自动追加 -2/-3 序号（整组原子，Preflight 橙色警告显式告知，不再阻塞）。Preflight 全部检查项至此实现
